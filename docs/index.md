---
layout: home
hero:
  name: Server Projects
  text: 服务器项目清单
  tagline: Ubuntu 22.04 (192.168.31.100) | 23 个项目 | 2026年6月
---

> 🕐 最后更新：2026-06-14 15:39

## 🟢 运行中的服务

| 服务 | 端口 | 地址 | 状态 |
|------|------|------|------|
| **portfolio** | 8000 | http://192.168.31.100:8000 | ✅ 运行中 |
| **vestio** | 8001 | http://192.168.31.100:8001 | ✅ 运行中 |

## 📚 VitePress 文档

通过 GitHub Pages 自动部署的在线文档：

| 项目 | 功能描述 | 在线文档 |
|------|----------|----------|
| **[ai-for-textile-procurement](https://github.com/dirjaker/ai-for-textile-procurement)** | 纺织采购 AI 学习指南与面试题库 | [在线访问](https://dirjaker.github.io/ai-for-textile-procurement/) |
| **[llm_agent_interview](https://github.com/dirjaker/llm_agent_interview)** | LLM & Agent 面试题全集（21专题/380+题/41000+行） | [在线访问](https://dirjaker.github.io/llm_agent_interview/) |
| **[hermes-skill-guide](https://github.com/dirjaker/hermes-skill-guide)** | Hermes Skill 开发指南（8章/20道面试题/5个示例） | [在线访问](https://dirjaker.github.io/hermes-skill-guide/) |
| **[docker_learning](https://github.com/dirjaker/docker_learning)** | Docker & Kubernetes 从零到实战（11篇/6示例/3速查表） | [在线访问](https://dirjaker.github.io/docker_learning/) |
| **[langchain_learning](https://github.com/dirjaker/langchain_learning)** | LangChain + LangGraph 学习项目 | [在线访问](https://dirjaker.github.io/langchain_learning/) |
| **[python_interview](https://github.com/dirjaker/python_interview)** | Python 面试题精讲 | [在线访问](https://dirjaker.github.io/python_interview/) |
| **[server-projects](https://github.com/dirjaker/server-projects)** | 服务器项目清单（本项目） | [在线访问](https://dirjaker.github.io/server-projects/) |

## 💻 应用项目

| 项目 | 功能描述 | 技术栈 | GitHub |
|------|----------|--------|--------|
| **[vestio](https://github.com/dirjaker/vestio)** | 智慧衣橱系统：衣物管理、AI搭配推荐、通义千问VL图片分类 | Vue3 + FastAPI + PyTorch | ✅ |
| **[yolo-trainer](https://github.com/dirjaker/yolo-trainer)** | YOLO 视觉模型训练平台（v5/v8/v9/v10） | Vue3 + FastAPI + PyTorch | ✅ |
| **[portfolio](https://github.com/dirjaker/portfolio)** | 个人作品集/项目展示网站 | FastAPI + SQLite + Jinja2 | ✅ |
| **[agent_platform](https://github.com/dirjaker/agent_platform)** | AI Agent 工具调用平台：Function Calling、ReAct循环、18种工具 | FastAPI + Pydantic | ✅ |
| **[agent_evaluator](https://github.com/dirjaker/agent_evaluator)** | Agent 评估框架：5维指标、自动化测试、A/B对比 | FastAPI + Pydantic | ✅ |
| **[agent_guardrails](https://github.com/dirjaker/agent_guardrails)** | Agent 安全防护：输入验证、PII脱敏、速率限制、审计日志 | FastAPI + Pydantic | ✅ |
| **[agent_memory_system](https://github.com/dirjaker/agent_memory_system)** | Agent 记忆系统：分层记忆、向量检索、记忆压缩与衰减 | FastAPI + SQLAlchemy | ✅ |
| **[multi_agent_crew](https://github.com/dirjaker/multi_agent_crew)** | 多智能体协作系统：3种协作模式、LLM驱动决策 | FastAPI + Pydantic | ✅ |
| **[workflow_engine](https://github.com/dirjaker/workflow_engine)** | AI 工作流编排引擎：DAG调度、7种节点、条件分支、并行执行 | FastAPI + SQLAlchemy | ✅ |
| **[model-monitor](https://github.com/dirjaker/model-monitor)** | 模型 API 监控：多协议适配、流量分析、费用追踪、告警 | FastAPI + SQLAlchemy | ✅ |
| **[model_deploy](https://github.com/dirjaker/model_deploy)** | 本地模型部署与推理优化：量化、vLLM、OpenAI兼容API | vLLM + Ollama | ✅ |
| **[api_doc_generator](https://github.com/dirjaker/api_doc_generator)** | API 文档生成器：AST代码解析、多格式输出、在线预览 | FastAPI + Python AST | ✅ |
| **[docker_optimizer](https://github.com/dirjaker/docker_optimizer)** | Docker 镜像优化器：层分析、问题检测、Dockerfile重写 | Click + Rich | ✅ |
| **[prompt_engineering](https://github.com/dirjaker/prompt_engineering)** | Prompt 工程平台：模板管理、版本控制、A/B测试、评估引擎 | FastAPI + SQLAlchemy | ✅ |
| **[damai_monitor](https://github.com/dirjaker/damai_monitor)** | 大麦网票务监控：实时监控、有票提醒、Chrome+Android双模式 | Playwright + Android | ✅ |
| **[knowledge_graph](https://github.com/dirjaker/knowledge_graph)** | 知识图谱系统：实体抽取、关系抽取、图谱可视化、NL问答 | FastAPI + Neo4j + D3.js | ✅ |

## 📊 项目统计

| 分类 | 数量 | 说明 |
|------|------|------|
| VitePress 文档 | 7 | 自动部署到 GitHub Pages |
| 应用项目 | 16 | FastAPI / Vue3 / tkinter |
| **合计** | **23** | |

## 🏗️ 技术栈总览

| 技术 | 用途 | 涉及项目数 |
|------|------|-----------|
| **Python 3.12** | 主要开发语言 | 23 |
| **FastAPI** | REST API 框架 | 17 |
| **Vue 3** | 前端框架 | 3 |
| **VitePress** | 文档站点生成器 | 7 |
| **SQLAlchemy** | ORM 数据库操作 | 12 |
| **PyTorch** | 深度学习框架 | 3 |
| **tkinter** | 桌面 GUI | 13 |
| **SQLite** | 轻量数据库 | 15 |
| **Docker** | 容器化部署 | 全部 |

## 🖥️ 服务器环境

| 项目 | 值 |
|------|-----|
| **操作系统** | Ubuntu 22.04 LTS |
| **内核版本** | 5.15.0-181-generic |
| **Python 版本** | 3.12.13 (Miniconda) |
| **服务器 IP** | 192.168.31.100 (内网) |
| **公共 IP** | 223.167.62.86 |
| **反向代理** | frpc → 101.132.81.140:7000 |
| **代理工具** | ShellCrash v1.9.4 |
| **包管理** | conda + pip (清华源) |
